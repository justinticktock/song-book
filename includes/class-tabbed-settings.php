	...
	class Tabbed_Settings extends Extendible_Tabbed_Settings {
		...
		public function field_page_select_list_option( array $args  ) {
			...
			wp_dropdown_pages( array( 
									...
									'show_option_none' => _x( "- None -", 'text for no page selected', 'song-book' ), 
									'option_none_value' => '0', 
		...
		public function field_plugin_checkbox_option( array $args  ) {
			...
			if ( ! file_exists( $plugin_main_file ) ) {
				...
				echo esc_html__( 'Force Active', 'song-book' ) . ' ( ';
				if ( ! $value ) { 
                                    echo '<a href="plugins.php?s=' . esc_html( $option['label'] )	 . '">' .  _x( 'Deactivate', 'deactivate the plugin', 'song-book' ) . "</a> | " ;	
                                }
			} else {
				echo esc_html__( 'Force Active', 'song-book' ) . ' ( ';
			}
			echo ' <a href="http://wordpress.org/plugins/' . esc_html( $option['slug'] ) . '">' .  esc_html__( "wordpress.org", 'song-book' ) . " </a> )" ;		
			?></label><?php
			if ( ! empty( $option['desc'] ) ) {
				echo ' <p class="description">' .  $option['desc']  . '</p>';
                        }
